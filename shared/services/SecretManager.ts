...
export class SecretManager extends pulumi.ComponentResource {
    ...
    constructor(
        ...
        super("custom:SharedSecretManager", name, {}, opts);

        const envVars = this.loadEnvFile(args.envFilePath);
        this.secrets = {};

        for (const [key, value] of Object.entries(envVars)) {
            const secretName = this.formatSecretName(`shared-${key}`);
    ...
    private createSecret(
        ...
        const secret = new gcp.secretmanager.Secret(name, {
            ...
            replication: {
                userManaged: {
                    replicas: [{ location: "us-central1" }], // TODO: pass as config rather than hardcode.
                },
            },
            project: projectId,
